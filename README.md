...
若要讓「產生家長連結」給異地家長直接開啟，服務必須部署在公開網址，或至少設定 `PUBLIC_BASE_URL` 為外部可連線的網址。只在本機啟動 `http://localhost:3000` 時，產生的連結仍然只能本機使用。
...
部署到 Render 時，家長連結會優先使用 Render 自動提供的 `RENDER_EXTERNAL_URL`。如果你之後綁定自訂網域，再另外設定 `PUBLIC_BASE_URL=https://你的正式網域` 即可讓家長連結改用正式網域。
...
設定後，老師端產生的家長連結會直接使用這個公開網址，而不是目前瀏覽器的 `localhost`。
